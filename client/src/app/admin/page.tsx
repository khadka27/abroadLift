...
export default function AdminPanel() {
  ...
  const stats = useMemo(() => {
    ...
    const totalApps = students.reduce(
      (acc, s) => acc + s.applications.length,
      0,
    );
    const avgVisaRate =
      totalStudents > 0
        ? Math.round(
            students.reduce(
              (acc, s) => acc + (s.visaChecks[0]?.successRate || 0),
              0,
            ) / totalStudents,
          )
        : 0;
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div className="space-y-2">
            ...
            <p className="text-slate-500 font-medium text-lg max-w-xl">
              Monitor candidate progression, admission applications, and visa
              probability metrics in real-time.
            </p>
          </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {student.applications.length > 0 ? (
                            <Badge className="bg-emerald-50 text-emerald-600 border-none shadow-none font-bold">
                              {student.applications.length} Active App
                        ...
                        {student.visaChecks.length > 0 ? (
                          <div className="flex items-center gap-2">
                            <div className="text-sm font-black text-slate-900">
                              {student.visaChecks[0].successRate}%
                            </div>
                            <div className="flex-1 w-20 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${student.visaChecks[0].successRate}%`,
                                }}
                              />
                            </div>
                          </div>
          ...
          {selectedStudent ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/5 rounded-3xl p-5 space-y-1">
                    ...
                    <p className="font-bold text-lg">
                      {selectedStudent.profile?.nationality || "N/A"}
                    </p>
                  </div>
                  <div className="bg-white/5 rounded-3xl p-5 space-y-1">
                    <p className="text-[10px] font-black text-white/30 uppercase tracking-widest">
                    ...
                    <p className="font-bold text-lg">
                      {selectedStudent.profile?.gpa || "N/A"}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    {selectedStudent.applications.map((app) => (
                      <div
                        key={app.id}
                        className="flex items-center justify-between bg-white/5 p-4 rounded-2xl border border-white/5 group hover:bg-white/5 transition-colors"
                      >
                        <div>
                          <p className="font-bold text-sm leading-tight group-hover:text-blue-400 transition-colors">
                            {app.university.name}
                          </p>
                          <p className="text-[10px] text-white/40 font-bold mt-1 uppercase">
                            {app.university.country}
                          </p>
                        </div>
                        <Badge
                          className={`border-none ${app.status === "ACCEPTED" ? "bg-emerald-500/10 text-emerald-400" : "bg-blue-500/10 text-blue-400"} font-black text-[9px] px-2.5`}
                        >
                          {app.status}
                        </Badge>
                      </div>
                    ...
                    {selectedStudent.applications.length === 0 && (
                      <p className="text-center text-white/30 text-xs font-bold py-4 italic border-2 border-dashed border-white/5 rounded-3xl">
                        No admission processes initiated.
                      </p>
                    )}
                  </div>
                </div>
                    ...
                    {selectedStudent.visaChecks.map((check) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-black ${check.successRate >= 70 ? "bg-emerald-500/20 text-emerald-400" : "bg-amber-500/20 text-amber-400"}`}
                          >
                            {check.successRate}%
                          </div>
                          <div>
            ...
            <div className="hidden xl:flex w-[450px] h-[700px] border-2 border-dashed border-slate-200 rounded-[40px] flex-col items-center justify-center p-12 text-center text-slate-400">
              ...
              <p className="text-sm font-medium leading-relaxed">
                Select a student from the active registry to view their
                comprehensive academic and immigration profile.
              </p>
            </div>
          )}
        </div>
