...
export default function AdminDashboard() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
      ...
      try {
          ...
          if (res.ok) {
              setShowModal(false);
              setFormData({ name: "", email: "", username: "", password: "", phoneNumber: "", role: "STUDENT" });
              fetchUsers();
          } else {
  ...
  const exportToExcel = () => {
    ...
    const headers = ["Name,Email,Username,Phone,Role,Nationality,CurrentCountry,RegisteredDate"];
    ...
    link.setAttribute("download", `abroadlift_users_${new Date().toISOString().slice(0,10)}.csv`);
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <aside className="w-[280px] bg-slate-900 hidden md:flex flex-col text-slate-400">
        <div className="p-8">
          <div className="flex items-center gap-3">
             ...
             <div className="flex flex-col">
                 <span className="font-black text-white tracking-widest text-[13px]">ABROADLIFT</span>
                 <span className="text-[10px] text-slate-500 font-bold tracking-widest leading-none mt-0.5 uppercase italic">Admin Engine</span>
             </div>
          </div>
        </div>

        <div className="px-6 mb-6">
            <p className="text-[10px] font-black tracking-[0.2em] uppercase text-slate-600 mb-4 px-4">Menu Section</p>
            <nav className="space-y-1.5">
              ...
              <button onClick={() => router.push("/")} className="w-full h-12 flex items-center gap-4 px-5 hover:bg-slate-800/50 hover:text-slate-200 rounded-[16px] font-bold text-[13px] transition-all">
        ...
        <div className="px-6 mt-auto mb-8">
            <p className="text-[10px] font-black tracking-[0.2em] uppercase text-slate-600 mb-4 px-4">Management</p>
            <nav className="space-y-1.5">
               <button className="w-full h-12 flex items-center gap-4 px-5 hover:bg-slate-800/50 hover:text-slate-200 rounded-[16px] font-bold text-[13px] transition-all">
                  <Settings className="w-4 h-4 text-slate-500" />
                  System Setup
               </button>
               <button onClick={() => signOut({ callbackUrl: "/" })} className="w-full h-12 flex items-center gap-4 px-5 group hover:bg-red-500/10 hover:text-red-400 rounded-[16px] font-bold text-[13px] transition-all">
        ...
        <div className="p-6 m-6 bg-slate-800/50 rounded-3xl border border-white/5">
             <div className="flex items-center gap-3 mb-3">
                 ...
                 <span className="text-[11px] font-black text-white tracking-tight uppercase italic">Growth Pack</span>
             </div>
             <p className="text-[10px] text-slate-500 font-bold leading-relaxed mb-4">Optimize your data processing workflow with AI intelligence.</p>
      ...
      <main className="flex-1 overflow-y-auto">
          ...
          <div className="flex items-center gap-6">
             ...
             <div className="flex items-center gap-4">
                <div className="text-right">
                    <p className="text-[13px] font-black text-slate-900 tracking-tight leading-none">{session?.user.name}</p>
                    <div className="flex items-center justify-end gap-1.5 mt-1">
                        <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                        <span className="text-[9px] font-black text-indigo-500 uppercase tracking-widest leading-none">System Admin</span>
                    </div>
                </div>
        ...
        <div className="p-10">
            <div className="flex items-center justify-between mb-10">
                <div>
                   <h1 className="text-[28px] font-black text-slate-900 tracking-tightest leading-tight">Infrastructure Control</h1>
                   <p className="text-sm font-bold text-slate-500 mt-1">Management overview for study abroad student ecosystem</p>
                </div>
                <div className="flex items-center gap-3">
                   <button 
            ...
            <div className="mb-10 grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                    { label: "Total Students", value: stats.student, sub: "+12.5% vs last month", icon: Users, color: "text-blue-500", light: "bg-blue-500/10", border: 'border-blue-200/50' },
                    { label: "Admin Access", value: stats.admin, sub: "High integrity keys", icon: Award, color: "text-indigo-500", light: "bg-indigo-500/10", border: 'border-indigo-200/50' },
                    { label: "Data Records", value: stats.total, sub: "Synced with server", icon: Globe, color: "text-emerald-500", light: "bg-emerald-500/10", border: 'border-emerald-200/50' },
                ].map((stat, i) => (
                    <div key={i} className={`bg-white p-8 rounded-[40px] border ${stat.border} shadow-sm group hover:-translate-y-1 transition-all duration-300`}>
                       <div className="flex items-center justify-between mb-6">
                           <div className={`w-14 h-14 ${stat.light} rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110`}>
                       ...
                       <div className="pt-5 border-t border-slate-50 flex items-center gap-2">
                           ...
                           <p className="text-[11px] font-bold text-slate-500">{stat.sub}</p>
                       </div>
                    </div>
                ))}
            </div>
            ...
            <div className="bg-white rounded-[45px] border border-slate-200 overflow-hidden shadow-sm shadow-slate-200/50">
               <div className="p-10 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                  <div className="flex items-center gap-6">
                     <h2 className="font-black text-slate-900 tracking-tight text-xl">Operational Directory</h2>
                     <div className="flex items-center gap-2 px-3 py-1 bg-white border border-slate-200 rounded-lg shadow-sm">
                        <span className="w-2 h-2 bg-emerald-500 rounded-full" />
                        <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest">{filteredUsers.length} Active Records</span>
                     </div>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <table className="w-full">
                     <thead>
                        <tr className="bg-white border-b border-slate-100">
                           <th className="px-10 py-6 text-left text-[11px] font-black text-slate-400 uppercase tracking-widest">Identified Student</th>
                           <th className="px-10 py-6 text-left text-[11px] font-black text-slate-400 uppercase tracking-widest">Digital Contact</th>
                           <th className="px-10 py-6 text-left text-[11px] font-black text-slate-400 uppercase tracking-widest">Verification</th>
                           <th className="px-10 py-6 text-left text-[11px] font-black text-slate-400 uppercase tracking-widest">Origin</th>
                           <th className="px-10 py-6 text-center text-[11px] font-black text-slate-400 uppercase tracking-widest">Control</th>
                        </tr>
                     </thead>
                     <tbody className="divide-y divide-slate-50">
                        {filteredUsers.map((user) => (
                           <tr key={user.id} className="hover:bg-slate-50/70 transition-all group">
                              <td className="px-10 py-7">
                                 <div className="flex items-center gap-5">
                                    ...
                                    <div>
                                       ...
                                       <div className="flex items-center gap-2">
                                          <span className="px-2 py-0.5 bg-slate-100 rounded-md text-[9px] font-bold text-slate-500 tracking-wide">ID: {user.id.slice(0, 8)}</span>
                              ...
                              <td className="px-10 py-7">
                                 <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black tracking-widest uppercase border shadow-sm ${
                                    user.role === "ADMIN" 
                              ...
                              <td className="px-10 py-7">
                                 <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-[13px] font-bold text-slate-800">{user.profile?.nationality || "Global User"}</span>
                                 </div>
                              </td>
                              <td className="px-10 py-7">
               ...
               {filteredUsers.length === 0 && (
                  <div className="p-10 text-center bg-white">
                     ...
                     <p className="text-lg font-black text-slate-900 uppercase italic">Missing Operational Records</p>
                     <p className="text-sm font-bold text-slate-400 mt-2">Adjust your query engine parameters to find specific student entities.</p>
                  </div>
               )}
               
               <div className="p-8 border-t border-slate-100 bg-slate-50/30 flex items-center justify-between">
                   <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest italic">AbroadLift Infrastructure Management v2.0</p>
                   <div className="flex items-center gap-1.5">
                       <span className="w-2 h-2 rounded-full bg-emerald-500" />
                       <span className="text-[10px] font-black text-slate-800 uppercase tracking-tighter">SERVER LINK STABLE</span>
                   </div>
               </div>
            </div>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-6 sm:p-0">
          <div onClick={() => setShowModal(false)} className="absolute inset-0 bg-slate-900/60 backdrop-blur-md" />
          <div className="relative w-full max-w-xl bg-white rounded-[50px] overflow-hidden shadow-[0_40px_100px_-20px_rgba(0,0,0,0.3)] border border-white/20 animate-in zoom-in-95 duration-300">
            ...
            <div className="bg-slate-900 p-10 flex items-center justify-between relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <h2 className="text-[26px] font-black text-white tracking-tightest leading-tight">Register Entity</h2>
                    <p className="text-slate-400 text-sm font-bold mt-1">Manual system enrollment for students</p>
                </div>
                <button onClick={() => setShowModal(false)} className="w-12 h-12 flex items-center justify-center rounded-3xl bg-white/5 hover:bg-white/10 text-white transition-all">
                    <X className="w-6 h-6" />
                </button>
            </div>
            ...
            <form onSubmit={handleRegister} className="p-10 space-y-6 bg-white">
                ...
                <div className="grid grid-cols-2 gap-5">
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Assigned Username</label>
                        <input 
                            type="text" 
                            required
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Digital Email</label>
                    <input 
                        type="email" 
                        required
                ...
                <div className="grid grid-cols-2 gap-5">
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Phone String</label>
                        <input 
                            type="text" 
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Infrastructure Role</label>
                    <select 
                ...
                <div className="pt-4">
                    ...
                    >
                        {registering ? "Initializing Record..." : "Confirm Enrollment"}
                    </button>
                    <p className="text-center text-[11px] font-bold text-slate-400 mt-6 uppercase tracking-widest">Global Audit Trail Applied to This Entry</p>
                </div>
            </form>
          </div>
        </div>
